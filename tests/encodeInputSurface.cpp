...
bool EncodeInputSurface::init(const char* inputFileName, uint32_t fourcc, int width, int height)
{
    ...
    CHECK_EQ(0, native_window_set_usage(GET_ANATIVEWINDOW(mNativeWindow),
             GRALLOC_USAGE_SW_READ_NEVER | GRALLOC_USAGE_SW_WRITE_OFTEN
             | GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_EXTERNAL_DISP));

    CHECK_EQ(0, native_window_set_scaling_mode(GET_ANATIVEWINDOW(mNativeWindow),
             NATIVE_WINDOW_SCALING_MODE_SCALE_TO_WINDOW));

    CHECK_EQ(0, native_window_set_buffers_geometry(mNativeWindow.get(),
                bufWidth, bufHeight, HAL_PIXEL_FORMAT_NV12_Y_TILED_INTEL));

    CHECK_EQ(0, native_window_set_buffer_count(GET_ANATIVEWINDOW(mNativeWindow), m_bufferCount));
